                    ...
                    for apt_index, apartment in enumerate(all_apartments):
                        ...
                        try:
                            ...
                            try:
                                ...
                                apartment_name = apartment.find('div', class_='property-type-for-rent').text.strip()

                        apartment_url = apartment.find('article')['data-url']
                        apartment_id = apartment.find('article')['data-listingid']

                        if apartment_url != 'https://www.apartments.com/299-webster-ave-new-rochelle-ny-unit-3f/pkc7q5v/':
                            continue
                        ...
                        if apartment_url not in processed_json_data:
                            ...
                            if 'Apartments For Rent' not in soup.title.text.strip():
                                ...
                                if not all_buildings:
                                    try:
                                        all_units = soup.find('div', id='priceBedBathAreaInfoWrapper').find('div', class_='priceBedRangeInfoContainer').find_all('ul')
                                    except:
                                        all_units = []

                                print(has_units, ' : ', len(all_units), ' : ', len(all_buildings))
                                ...
                                if not all_buildings and all_units:
                                    ...
                                    print('Trying All Units...')

                                    for unit in all_units:

                                        if not unit.find_all('li'):
                                            continue

                                        unit_num = ''
                                        unit_price = unit.find_all('li')[0].find('p', class_='rentInfoDetail').text.strip().replace(',', '').replace('$', '')
                                        unit_size = unit.find_all('li')[3].find('p', class_='rentInfoDetail').text.strip()
                                        unit_available = ''

                                        bed = unit.find_all('li')[1].find('p', class_='rentInfoDetail').text.strip().split()[0].strip()
                                        bath = unit.find_all('li')[2].find('p', class_='rentInfoDetail').text.strip().split()[0].strip()
